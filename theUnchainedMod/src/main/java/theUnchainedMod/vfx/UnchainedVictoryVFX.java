...
public class UnchainedVictoryVFX extends AbstractGameEffect {
    ...
    public UnchainedVictoryVFX() {
        ...
        this.startX = Settings.WIDTH/2f;
        this.startY = Settings.HEIGHT/2f;
    ...
    public void update() {
        ...
        if(glowingEyeOpacity > 0)
        {
            ...
            if(glowingEyeOpacity < 0) {
                ...
                /*AbstractDungeon.effectsQueue.add(new ChainAcrossScreenEffect(color, MathUtils.random(0, 360)));
                AbstractDungeon.effectsQueue.add(new ChainAcrossScreenEffect(color, MathUtils.random(0, 360)));
                AbstractDungeon.effectsQueue.add(new ChainAcrossScreenEffect(color, MathUtils.random(0, 360)));
                AbstractDungeon.effectsQueue.add(new ChainAcrossScreenEffect(color, MathUtils.random(0, 360)));*/
            }
    ...
    public void render(SpriteBatch sb) {
        ...
        sb.draw(glowImage, this.startX, this.startY, (float)glowImage.packedWidth / 2.0F, (float)glowImage.packedHeight / 2.0F, (float)glowImage.packedWidth, (float)glowImage.packedHeight, 5* scaleX, 5* scaleY, this.rotation);
